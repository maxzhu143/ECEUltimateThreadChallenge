...
char callbackURItemplate[] = "%s%s";                            // only 2 parts now, no IP slot
char callbackURIProtocol[] = "http%3A%2F%2F127.0.0.1%3A8888"; // http://127.0.0.1:8888
...
bool launchRefreshTokenFlow(SpotifyArduino *spotifyObj, char *clientId, char *tokenBuffer){
  refreshTokenBuffer = tokenBuffer;  // point to main's buffer
  ...
  sprintf(callbackURI, callbackURItemplate, callbackURIProtocol, callbackURIAddress);
#endif
